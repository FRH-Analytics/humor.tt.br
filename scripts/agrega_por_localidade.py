...
def capturalocalidade(linha):
  linha_campos_localidade = linha.split('\t')[4]
  return linha_campos_localidade.replace('\n','').replace('\t','').replace('"','').strip()

if len(sys.argv) != 3:
  print "Uso: python agrega_por_localidade.py <arquivo com tweets classificados> <arquivo de saida>"
  sys.exit()
